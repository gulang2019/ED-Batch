...
bool PQTree::TemplateL1(PQNode* candidate_node) {
  ...
  if (candidate_node->parent_){
    dirty_nodes_.insert(candidate_node->parent_);
    // string str;
    // candidate_node->Print(&str);
    // str = "";
    // candidate_node->parent_->Print(&str);
    // fprintf(stdout, "[insert_dirty %s, %d]: %p\n", __FILE__, __LINE__, candidate_node->parent_);
  }
...
bool PQTree::TemplateP1(PQNode* candidate_node, bool is_reduction_root) {
  ...
  if (candidate_node->type_ != PQNode::pnode ||
      (int)candidate_node->full_children_.size() != candidate_node->ChildCount())
    return false;
...
bool PQTree::Bubble(const set<int>& reduction_set, bool create_pseudonode) {
  queue<PQNode*> q;
  block_count_ = 0;
  blocked_nodes_ = 0;
  off_the_top_ = 0;
  ...
  while (q.size() + block_count_ + off_the_top_ > 1) {
    ...
    dirty_nodes_.insert(candidate_node);
    // fprintf(stdout, "[insert_dirty %s, %d]: %p\n", __FILE__, __LINE__, candidate_node);
    ...
    if (candidate_node->mark_ == PQNode::unblocked) {
      ...
      if (!candidate_node->parent_) {
        ...
        if (root == nullptr && candidate_node->parent_->pertinent_child_count == reduction_set.size()){
          root = candidate_node->parent_;
        }
  ...
  if (!q.empty()){
    dirty_nodes_.insert(q.front());
    // fprintf(stdout, "[insert_dirty %s, %d]: %p\n", __FILE__, __LINE__, q.front());
  }
  ...
  if (block_count_ == 1 && blocked_nodes_ > 1) {
    if (create_pseudonode){
      ...
      auto current = *blocked_list.begin();
      assert(current->ImmediateSiblingCount() == 2);
      auto last = current->immediate_siblings_[0];
      while (current->ImmediateSiblingCount() == 2){
        auto next = current->QNextChild(last);
        last = current;
        current = next;
      }
      auto parent = current->parent_;
      parent->pertinent_child_count = blocked_list.size();
      for (auto node: blocked_list) node->parent_ = parent; 
      dirty_nodes_.insert(parent);
    }
  }
  return true;
}
...
void PQTree::Reset(){
  for (auto node: dirty_nodes_){
    string str;
    node->Print(&str);
    node->full_children_.clear();
    node->partial_children_.clear();
    node->label_                 = PQNode::empty;
    node->mark_                  = PQNode::unmarked;
    node->pertinent_child_count = 0;
    node->pertinent_leaf_count  = 0;
    node->pseudochild_           = false;
    node->pseudonode_            = false;
  }
  dirty_nodes_.clear();
}

bool PQTree::ReduceStep(const set<int>& reduction_set, PQNode ** root) {
  ...
  while (!q.empty()) {
    ...
    string str;
    candidate_node->Print(&str);
    // fprintf(stdout, "[ReduceStep]: %p %s %d %p\n ", candidate_node, str.c_str(),
    // candidate_node->pertinent_leaf_count < (int)reduction_set.size(), pseudonode_);

    // We test against different templates depending on whether |candidate_node|
    // is the root of the pertinent subtree.
    if (candidate_node->pertinent_leaf_count < (int)reduction_set.size()) {
...
void PQTree::CleanPseudo() {
  if (pseudonode_) {
    ...
    dirty_nodes_.erase(pseudonode_);
    delete pseudonode_;
    pseudonode_ = NULL;
  }
}
...
PQTree::PQTree(set<int> reduction_set) {
  ...
  for (set<int>::iterator i = reduction_set.begin();
    ...
    dirty_nodes_.insert(new_node);
    // fprintf(stdout, "[insert_dirty %s, %d]: %p\n", __FILE__, __LINE__, new_node);
...
bool PQTree::SafeReduce(const set<int>& S) {
...
bool PQTree::SafeReduceAll(const list<set<int> >& L) {
  //using a backup copy to enforce safety
  PQTree toCopy(*this);
  if (!ReduceAll(L)) {
    //reduce failed, so perform a copy
...
bool PQTree::Reduce(const set<int>& reduction_set) {
  if (reduction_set.size() < 2) {
    reductions_.push_back(reduction_set);
    return true;
  }
  // fprintf(stdout, "------------before reduce--------\n");
  // root_->CheckReset();
  // fprintf(stdout, "---------------init--------------\n");
  if (invalid_)
    return false;
  if (!Bubble(reduction_set)) {
    // invalid_ = true;
    Reset();
    // cout << "invalid when bubble" << endl;
    return false;
  }
  if (!ReduceStep(reduction_set)) {
    // invalid_ = true;
    Reset();
    // cout << "invalid when reduce" << endl;
    return false;
  }
...
bool PQTree::ReduceAll(const list<set<int> >& L) {
  for (auto & S:L) {
    if (!Reduce(S))
      return false;
  }
...
bool PQTree::ReduceAll(const vector<set<int> >& L){
  for (auto & S: L){
...
bool PQTree::ReduceAll(const set<set<int> >& L) {
  for (set<set<int> >::iterator S = L.begin(); S != L.end(); S++) {
    if (!Reduce(*S)){
      // fprintf(stdout, "%s\n", Print().c_str());
