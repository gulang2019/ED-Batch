...
bool PQNode::ConsecutiveFullPartialChildren() {
  ...
  if (counts[partial] != (int)partial_children_.size())
    return false;
  // Depending on how many partials there are, most full children will get
  // counted twice.
  if (counts[full] != (int)((full_children_.size() * 2) - (2 - counts[partial])))
    return false;
  return true;
}
...
void PQNode::CheckReset() {
  ...
  if (full_children_.size() || 
  partial_children_.size() || 
  label_                 != empty ||
  mark_                  != unmarked || 
  pertinent_child_count != 0 ||
  pertinent_leaf_count  != 0 ||
  ...
  pseudonode_            ){
    string str;
    Print(&str);
    fprintf(stdout, "[Reset]: %s is not reset\n", str.c_str());
  }
  
  assert(label_ == empty);
  assert(mark_ == unmarked);
  assert(pertinent_child_count == 0);
  assert(pertinent_leaf_count == 0);
  assert(!pseudochild_);
  assert(!pseudonode_);
  assert(partial_children_.size() == 0);
  assert(full_children_.size() == 0);
}
...
QNodeChildrenIterator::QNodeChildrenIterator(
  ...
  parent_ = parent;
  assert(parent_->type_ == PQNode::qnode);
  Reset(begin_side);
}
