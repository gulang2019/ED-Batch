...
class PQNode {
 ...
 public:
  ...
  void Children(std::vector<PQNode*> *children);
 ...
 private:
  ...
  std::list<PQNode*> circular_link_;
...
// std::list.  Each child contains pointers to the 1 or two siblings on either side
...
class QNodeChildrenIterator {
 public:
  ...
  // Returns a pointer to the current PQNode child in the std::list.
  PQNode* Current();

  // Next advances the current position in the child std::list.  If |IsDone()|,
  // operation has no effect.
  void Next();

  // Resets the iterator to the beginning of the std::list of children, optionally
  ...
  // std::list order (forward or reverse) is consistent during the life of the
  // object.
  bool IsDone();
